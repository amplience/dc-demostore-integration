...
export class FabricCommerceCodec extends CommerceCodec {
	...
	async cacheMegaMenu(): Promise<void> {
		...
		const categories: any[] = _.get(await this.fetch('/api-category/v1/category?page=1&size=1&type=ALL'), 'categories[0].children')
		if (!categories) {
	...
	async getProducts(args: GetProductsArgs): Promise<Product[]> {
		...
		if (args.productIds) {
			...
			const skus = _.take(_.get(await this.fetch(`/api-category/v1/category/sku?id=${args.category.id}`), 'skus'), 20)
