...
import { CommerceAPI, CustomerGroup, GetCommerceObjectArgs, GetProductsArgs, Category, Product, CommonArgs, GetProductVariantsArgs } from '../../../common'
...
const sfccCodec: CommerceCodec = {
	...
	getAPI: async (config: CodecPropertyConfig<CodecConfig>): Promise<CommerceAPI> => {
		...
		const api = (args: CommonArgs) => ({
			getCategory: async (slug = 'root'): Promise<Category> => {
			...
			search: async (query: string): Promise<Product[]> => {
				const searchResults = (await fetch(`${shopApi}/product_search?${query}`)).hits
				if (searchResults) {
			...
			getVariants: async (id: string): Promise<SFCCProduct> => {
				return await fetch(`${shopApi}/products/${id}/variations`)
			},
		...
		return {
			...
			getVariants: async function (args:GetProductVariantsArgs): Promise<SFCCProduct> {
				return await api(args).getVariants(args.productId)
			},
			getCategory: async function (args: GetCommerceObjectArgs): Promise<Category> {
				// let category = await api(args).getCategory(args.slug)
				const category = findInMegaMenu(megaMenu, args.slug)
				if (category) {
					return {
						...category,
