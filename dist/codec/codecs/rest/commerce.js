    ...
    getProducts(context) {
        return __awaiter(this, void 0, void 0, function* () {
            let products = context.args.productIds && lodash_1.default.filter(this.products, prod => context.args.productIds.split(',').includes(prod.id)) ||
                // context.args.keyword && _.filter(this.products, prod => containsLowerCase([prod.name], context.args.keyword))
            ...
            if (context.args.keyword) {
                console.log(`keyword: ${context.args.keyword} returns ${products.length} results`);
            }
            return {
